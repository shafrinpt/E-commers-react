import lipstickImg from "../assets/images/lipsticks.jpg";
import pixi from "../assets/images/makeup/blush-pixi.jpg";
import blush from "../assets/images/makeup/blush.jpg";
import fenty from "../assets/images/makeup/concealer-fenty.jpg";
import dior from "../assets/images/makeup/dior-found.jpg";
import elf from "../assets/images/makeup/elf-eye.jpg";
import eyeShadow from "../assets/images/makeup/eye-shadow.jpg";
import  kay from "../assets/images/makeup/kay-lip.jpg";
import  lakme from "../assets/images/makeup/lakme-lip.jpg";
import  mac from "../assets/images/makeup/mac-com.jpg";
import  maskara from "../assets/images/makeup/maskara.jpg";
import  lipliner from "../assets/images/makeup/lip-liner.jpg";



const makeup = [
  {
    id: 1,
    name: "M.A.C MACximal Matte-Lipstick",
    price: 2550,
    brand: "M.A.C",
    category: "Lips",
    image: lipstickImg,
    description: "Long-lasting matte lipstick with rich color payoff.",
  },
  {
    id: 2,
    name: "Pixi By Petra On The Glow Blush",
    price: 1950,
    brand: "Pixi",
    category: "Cheeks",
    image: pixi,
    description: "Lightweight full-coverage foundation for all-day wear.",
  },
  {
    id: 3,
    name: "Waterproof EyelinerSoft Pinch Liquid Blush",
    price: 2219,
    brand: "Rare Beauty",
    category: "Cheeks",
    image: blush,
    description: "Precision tip eyeliner for perfect winged looks.",
  },
  {
    id: 4,
    name: "Fenty Beauty Longwear Concealer",
    price: 3425,
    brand: "Fenty Beauty",
    category: "Face",
    image: fenty,
    description: "Fenty Beauty Pro Filt'r Instant Retouch Concealer.",
  },
  {
    id: 5,
    name: "DIOR Backstage Face & Body Foundation",
    price: 4700,
    brand: "DIOR",
    category: "Face",
    image: dior,
    description: "The go-to face and body foundation for House of DIOR runway shows.",
  },
  {
    id: 6,
    name: "e.l.f. Cosmetics Expert Liquid Eyeliner",
    price: 360,
    brand: "e.l.f",
    category: "Eyes",
    image:elf,
    description: "pulling on your eyelids with this easy glide e.l.f Expert Liquid Liner.",
  },
   {
    id: 7,
    name: "Kay Beauty Multi Texture Eyeshadow Palette ",
    price: 1799,
    brand: "Kay Beauty",
    category: "Eyes",
    image: eyeShadow,
    description: "A Night Glam Palette Thats Perfect for High Impact eye look.",
  },
  {
    id: 8,
    name: "Kay Beauty Hydra Creme Lipstick",
    price: 899,
    brand: "Kay Beauty",
    category: "Lips",
    image:kay,
    description: "our most hydrating lip colour ever.",
  },
   {
    id: 9,
    name: "Lakme 9 To 5 Priming Matte Lipstick",
    price: 599,
    brand: "Lakme",
    category: "Lips",
    image: lakme,
    description: "Super Pigmented Shades with an instense colour payoff.",
  },
  {
    id: 10,
    name: "M.A.C Studio Fix Powder ",
    price: 3025,
    brand: "M.A.C",
    category: "Face",
    image:mac,
    description: "New Blur-Matte Finish 2-in-1 foundation & powder.",
  },
   {
    id: 11,
    name: "Volumizing Mascara",
    price: 599,
    brand: "Maybelline",
    category: "Eyes",
    image: maskara,
    description: "Smudge-proof mascara for fuller, longer lashes.",
  },
  {
    id: 12,
    name: "e.l.f. Cosmetics Cream Glide Lip Liner",
    price: 300,
    brand: "e.l.f",
    category: "Lips",
    image:lipliner,
    description: "Line, define your lips with e.l.f. Cosmetics Cream Glide Lip Liner.",
  },
];

export default makeup;